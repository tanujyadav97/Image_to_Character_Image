...
# Change this to 0 if you don't want to resize the image before conversion
resize = 1
...
if __name__ == '__main__':
    ...
    image = Image.open("original-images/boy.jpg", "r")
    ...
    for i in range(0, mat.shape[0]):
        for j in range(0, mat.shape[1]):
            if invert_color:
                newimage += (point[int(mat[i][j] / 19)])+(point[int(mat[i][j] / 19)])
            else:
                newimage += (point[int((255 - mat[i][j]) / 19)])+(point[int((255 - mat[i][j]) / 19)])
        newimage += '\n'
